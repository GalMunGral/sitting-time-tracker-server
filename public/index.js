...
d3.json(`http://localhost:8080/record?token=${token}`)
...
function draw(records) {
  ...
  _.range(totalDays).forEach(i => {
    _.range(24).forEach(j => {
      placeHolder.append('rect')
        .attr('y', d => `calc(${i} * 100vw/24 + 2)`)
        .attr('x', d => `calc(${j} * 100vw/24 + 2)`)
        .attr('height', `calc(100vw/24 - 4)`)
        .attr('width', `calc(100vw/24 - 4)`)
        .attr('fill', 'lightgrey')
    })
  })
  ...
  svg.selectAll('svg > rect')
    ...
    .enter()
    .append('rect')
    .attr('y', d => `calc(${Math.floor(d.key/24) - Math.floor(startHour/24)} * 100vw/24 + 2)`)
    .attr('x', d => `calc(${d.key % 24} * 100vw/24 + 2)`)
    .attr('height', `calc(100vw/24 - 4)`)
    .attr('width', `calc(100vw/24 - 4)`)
